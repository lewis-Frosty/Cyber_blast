...
// Copied from src/core/gameState.ts by scripts/build-edge-shared.mjs so the Edge Function
// runs the SAME logic as the client. Edit the source, then re-run the script.

import { Board } from './Board.ts';
import { resolveBoard, type CascadeResult, type Generation0 } from './cascade.ts';
import { isObstacle, OBSTACLE_PIECE, spawnPiece, SHAPES, type Piece, type Shape } from './Piece.ts';
import { colourMultiplier, scoreTurn, type ScoreBreakdown } from './scoring.ts';
import { createRng, type Rng } from './rng.ts';
import { createStats, recordPlacement, type GameStats } from './stats.ts';
import {
  addCharge,
  colourHasPowerUp,
  connectedRegion,
...
import type { CellIndex, ColorId } from './types.ts';
