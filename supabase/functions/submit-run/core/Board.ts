...
// Copied from src/core/Board.ts by scripts/build-edge-shared.mjs so the Edge Function
// runs the SAME logic as the client. Edit the source, then re-run the script.

import { BLOCKED, EMPTY, type Cell, type CellIndex, type ColorId, type Coord } from './types.ts';
import type { Shape } from './Piece.ts';
